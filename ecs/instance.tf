...
resource "alicloud_security_group" "group" {
  count                 = "${var.use_ecs_module ? (var.ecs_count != 0 ? 1 : (var.delete_protection ? 1 : 0)) : 0}"
...
resource "alicloud_instance" "instance" {
  count = "${var.use_ecs_module ? (var.ecs_count != 0 ? var.ecs_count : (var.delete_protection ? 1 : 0)) : 0}"
  instance_name = "${var.ecs_name}-${format(var.count_format, count.index+1)}"
